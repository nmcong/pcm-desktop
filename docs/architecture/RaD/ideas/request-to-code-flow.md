...
This document illustrates how the PCM Desktop application can implement the pipeline that maps a user requirement to the
exact source-code location(s) that need investigation or modification. It
complements `rag-strategy.md`, `function-calling-spec.md`, and `ast-source-analysis.md`.
...
3. Each snippet references the file path and line numbers so users can jump to the file via the file viewer/IDE
   integration.
...
- `ChunkHit.metadata` should always include `project_id`, `file_id`, `relative_path`, `start_line`, `end_line`, and
  optionally `node_id` to enable AST enrichment.
- `AstContextService` must use the same snapshot/version as the retrieval chunk (store `snapshot_id` in metadata) to
  avoid mismatches.
...
This skeleton can be adapted to Java/Spring, Kotlin, or any backend stack and ensures the codebase mirrors the
architecture defined in the RaD documents.
