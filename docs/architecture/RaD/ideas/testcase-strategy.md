...
This document describes how PCM Desktop can automate and guide test-case creation whenever source code changes are analyzed. It complements `ast-source-analysis.md`, `request-to-code-flow.md`, and the broader RAG architecture.
...
- **Impact analysis output** (`ImpactAnalyzer` from `request-to-code-flow.md`): list of affected files, methods, and dependencies.
- **Existing test metadata**: map test cases → code coverage (if available) or `test_catalog` table (optional future feature).
...
4. **Assess system impact**: If the changed code is part of shared services, include integration/E2E tests covering downstream systems.
...
By following this strategy, the system can respond to code changes with targeted, high-quality test recommendations that account for both structural impact and edge-case coverage.
