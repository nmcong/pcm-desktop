...
This guide explains how to extract content from Microsoft Compiled HTML Help (CHM) files and feed it into the PCM
Desktop knowledge system so the RAG pipeline can reference the material.
...
- Once extracted, the resulting HTML/text can be treated like any other knowledge article (stored in SQLite, embedded in
  Qdrant, indexed via FTS5).
...
Recommendation: use `extract_chmLib` (Linux/macOS) or `hh.exe` (Windows). Wrap in a script so users can point to a CHM
file and produce a folder of HTML/TOC.
...
- **Versioning**: tie imports to `analysis_snapshots` or a dedicated `chm_imports` table with timestamp/commit so you
  can trace which CHM version supported a response.
...
After storing documents, insert their text into `search_corpus` (with `source_type='chm_doc'`) and chunk/embed them like
