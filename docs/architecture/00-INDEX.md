...
Bộ tài liệu Architecture này được tạo ra dựa trên phân tích chi tiết từ `docs/RaD/detail/`, cung cấp **blueprint hoàn
chỉnh** để implement PCM Desktop.
...
- Q: "Tôi cần implement screen mới, bắt đầu từ đâu?"
    - A: Read `02-Screen Structure` → Design layout → Read `03-Field Specifications` → Read `04-Architecture` §3-§4 →
      Implement
