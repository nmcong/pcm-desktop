...
createMutedLabel("Configure your preferences")
);
```
...
add(StyleConstants.CARD);
label.

getStyleClass().

add(StyleConstants.TEXT_MUTED);
```
...
setIconSize(LayoutConstants.ICON_SIZE_SM);
```
...
add(StyleConstants.BUTTON_PRIMARY);
```
