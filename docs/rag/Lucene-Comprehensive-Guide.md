...
**Apache Lucene** là một thư viện information retrieval mã nguồn mở được viết hoàn toàn bằng Java. Đây là foundation cho
nhiều search engines nổi tiếng như Elasticsearch, Solr, và Amazon CloudSearch.
...
setTypes(Set.of(DocumentType.SOURCE_CODE, DocumentType.TEST_FILE));
        options.

setFilters(Map.of("project", "customer-service"));
...
setTypes(Set.of(DocumentType.DOCUMENTATION, DocumentType.CONFIG_FILE));
        options.
...
updateDocument(idTerm, luceneDoc);
    writer.
...
error("Failed to index document: {}",document.getId(),e);
        throw new

VectorStoreException("Failed to index document: "+document.getId(),e);
        }
```
...
setRAMBufferSizeMB(128); // Reduce from default 256MB
config.

setMaxBufferedDocs(500);  // Reduce from default 1000
...
Apache Lucene là **strategic choice** đúng đắn cho PCM Desktop, cung cấp both **immediate value** và **long-term
extensibility** cho advanced search và AI capabilities.
