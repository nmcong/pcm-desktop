...
/**
 * Extracts tokens from Windows Registry Useful when SSO system stores tokens in registry
 *
 * @author PCM Team
 * @version 1.0.0
 */
...
public class RegistryExtractor {
        ...
        try {
            ...
            for (String registryPath : registryPaths) {
                Optional<String> token =
                        readRegistryValue(Preferences.userRoot(), registryPath, serviceName);
                if (token.isPresent()) {
                    return token;
                }
            }
    ...
    private Optional<String> extractFromMachineRegistry(String serviceName) {
        try {
            ...
            for (String registryPath : registryPaths) {
                Optional<String> token =
                        readRegistryValue(Preferences.systemRoot(), registryPath, serviceName);
                if (token.isPresent()) {
                    return token;
                }
            }
