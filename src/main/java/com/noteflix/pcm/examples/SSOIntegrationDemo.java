 ...
 *
 * <p>Demonstrates: - Token extraction from various sources - Token caching and management -
...
public class SSOIntegrationDemo {
  ...
  private static void tokenCacheManagement() {
    ...
    for (String service : cachedServices) {
      ...
      System.out.printf("  • %s: %s", service, token.isPresent() ? "✅ Valid" : "❌ Expired");
  ...
  private static void checkServiceStatus() {
    ...
    String[] testServices = {
      "demo-service", "openai-service", "company-portal", "anthropic-service", "internal-llm"
    };
    ...
    if (!customService.isEmpty()) {
      ...
      System.out.printf(
          "  • %-20s: %s\n",
          customService, authenticated ? "✅ Authenticated" : "❌ Not authenticated");
    }
  }
