...
public class CSSTestPage extends BasePage {
  ...
  private VBox createThemeControls() {
    ...
    Label currentTheme =
        new Label(
            "Current: " + (themeManager != null && themeManager.isDarkTheme() ? "Dark" : "Light"));
  ...
  private VBox createColorTestSection() {
    ...
    successLabel.setStyle(
        "-fx-background-color: -fx-success; -fx-text-fill: white; -fx-padding: 5px 10px;");

    Label warningLabel = new Label("Warning Color");
    warningLabel.setStyle(
        "-fx-background-color: -fx-warning; -fx-text-fill: white; -fx-padding: 5px 10px;");

    Label errorLabel = new Label("Error Color");
    errorLabel.setStyle(
        "-fx-background-color: -fx-error; -fx-text-fill: white; -fx-padding: 5px 10px;");
