...
/**
 * Batch Jobs page - Single Responsibility Principle
 * Manages batch job monitoring and scheduling
 */
@Slf4j
public class BatchJobsPage extends BasePage {
    ...
    private VBox createJobsTable() {
        ...
        statusColumn.setCellFactory(column -> new TableCell<JobEntry, String>() {
            @Override
            protected void updateItem(String item, boolean empty) {
                ...
                if (empty || item == null) {
                    ...
                    setText(item);
                    getStyleClass().removeAll("status-running", "status-completed", "status-failed", "status-scheduled");
        ...
        lastRunColumn.setCellValueFactory(data -> new javafx.beans.property.SimpleStringProperty(data.getValue().getLastRun()));
        lastRunColumn.setPrefWidth(150);

        TableColumn<JobEntry, String> nextRunColumn = new TableColumn<>("Next Run");
        nextRunColumn.setCellValueFactory(data -> new javafx.beans.property.SimpleStringProperty(data.getValue().getNextRun()));
        ...
        actionsColumn.setCellFactory(column -> new TableCell<JobEntry, Void>() {
