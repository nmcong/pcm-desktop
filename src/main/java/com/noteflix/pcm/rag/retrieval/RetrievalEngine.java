...
public class RetrievalEngine {
    ...
    public RetrievalEngine(VectorStore vectorStore, boolean enableQueryExpansion, boolean enableReranking) {
    ...
    public List<ScoredDocument> retrieve(String query, RetrievalOptions options) {
        log.debug("Retrieving documents for query: '{}' with {} max results", 
            query, options.getMaxResults());
        
        // 1. Optionally expand query
        List<String> queries = enableQueryExpansion ? expandQuery(query) : Collections.singletonList(query);
     ...
     */
    private List<ScoredDocument> applyDiversityFilter(List<ScoredDocument> results, RetrievalOptions options) {
        if (results.size() <= 3) {
