 ...
 *
 * <p>Handles direct SQL operations for conversations table.
 *
 * @author PCM Team
 * @version 1.0.0
 */
...
public class ConversationDAO implements DAO<Conversation, Long> {
  ...
  private static final String INSERT_SQL =
      "INSERT INTO "
          + TABLE_NAME
          + " (title, user_id, preview, llm_provider, llm_model, system_prompt, message_count,"
          + " total_tokens, archived, pinned, metadata, created_at, updated_at) VALUES (?, ?, ?, ?,"
          + " ?, ?, ?, ?, ?, ?, ?, ?, ?)";
  ...
  private static final String SEARCH_SQL =
      "SELECT * FROM "
          + TABLE_NAME
          + " WHERE user_id = ? AND (title LIKE ? OR preview LIKE ?) "
  ...
  public List<Conversation> search(String userId, String query, Connection connection)
      throws SQLException {
    List<Conversation> conversations = new ArrayList<>();
