 ...
 * 
 * Features:
 * - Automatic caching based on strategy
 * - Optional summarization to save tokens
    ...
    public ProcessedToolResult process(ToolExecutionContext context) {
        ...
        if (cached != null && !cached.isExpired()) {
            ...
            return ProcessedToolResult.builder()
                ...
                .displayResult(decision.isShouldSummarize() && cached.getSummary() != null
                    ? cached.getSummary() 
        ...
        if (decision.isShouldSummarize()) {
            ...
            log.debug("Summarized tool result: {} -> {} tokens (saved {})",
                originalTokens, summaryTokens, tokensSaved);
        } else {
            // Use full result
            displayResult = context.getRawResult() != null 
                ? context.getRawResult().toString()
                : "";
        }
    ...
    private String generateCacheKey(String toolName, Map<String, Object> args) {
        ...
        if (args != null) {
            args.entrySet().stream()
                ...
                .forEach(entry -> key.append(":").append(entry.getKey())
