 ...
 *
 * <p>Cách sử dụng: 1. Set environment variable: export OPENAI_API_KEY=your-key 2. Run: java APIDemo
...
public class APIDemo {
  ...
  private static void demoAdvancedRequest() {
    ...
    try {
      // Build request
      LLMRequest.LLMRequestBuilder requestBuilder =
          LLMRequest.builder().model("gpt-3.5-turbo").temperature(temperature).maxTokens(maxTokens);

      if (!systemMsg.isEmpty()) {
        requestBuilder.messages(List.of(Message.system(systemMsg), Message.user(userMsg)));
      } else {
  ...
  private static void demoConversation() {
    ...
    try {
      LLMService.ConversationBuilder conversation =
          llmService
              .newConversation()
  ...
  private static void demoFunctionCalling() {
    ...
    try {
      ...
      FunctionDefinition calcFunction =
