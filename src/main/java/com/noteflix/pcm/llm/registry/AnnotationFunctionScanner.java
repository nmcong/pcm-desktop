 ...
 * 
 * Uses reflection to:
 * 1. Find all classes annotated with @FunctionProvider
 * 2. Find all methods annotated with @LLMFunction in those classes
 * 3. Extract parameter metadata from @Param annotations
 * 4. Create RegisteredFunction wrappers
 * 5. Register functions in FunctionRegistry
 * 
 * Functions execute in the current application context,
 * so provider instances can have dependencies injected.
 */
@Slf4j
public class AnnotationFunctionScanner {
     ...
     * 
     * NOTE: Automatic package scanning requires org.reflections library.
     * For now, use scanClass() or register() methods to add functions manually.
     * 
        ...
        try {
            ...
            for (Method method : providerClass.getDeclaredMethods()) {
                if (method.isAnnotationPresent(LLMFunction.class)) {
                    ...
                    if (!annotation.enabled()) {
                        log.debug("Skipping disabled function: {}.{}", 
                            providerClass.getSimpleName(), method.getName());
                        continue;
                    }
                    
                    RegisteredFunction function = createFunctionFromMethod(
                        providerInstance, 
                        method, 
                        annotation
                    );
                    ...
                    log.debug("Registered function: {} from {}.{}", 
                        function.getName(), 
     ...
     * 
     * First checks DI container (Injector), then creates new instance.
     */
    ...
    private RegisteredFunction createFunctionFromMethod(
        Object instance,
        Method method,
        LLMFunction annotation
    ) {
        // Get function name
        String functionName = annotation.name().isEmpty() 
            ? camelToSnakeCase(method.getName())
            : annotation.name();
    ...
    private JsonSchema buildParameterSchema(Method method) {
        ...
        for (Parameter parameter : parameters) {
            ...
            if (paramAnnotation == null) {
                log.warn("Parameter without @Param annotation: {} in method {}",
    ...
    /**
     * Inner class: Wrapper for annotated functions.
     * Handles reflection-based method invocation.
     */
