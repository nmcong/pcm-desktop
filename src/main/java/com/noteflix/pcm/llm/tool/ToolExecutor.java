 ...
 *
 * <p>Supports: - Sequential execution (one after another) - Parallel execution (when dependencies
 * allow) - Error handling for individual tool failures - Execution timing and logging
 *
 * <p>Example usage:
 *
 * <pre>
...
public class ToolExecutor {
     ...
     * Execute all tool calls sequentially.
     *
     * <p>Tools are executed in order. If one fails, execution continues with remaining tools
     * (fail-fast is not used).
     *
    ...
    public ToolResult executeSingle(ToolCall call, int index) {
        ...
        try {
            ...
            return ToolResult.success(call.getId(), toolName, result, duration);

        } catch (FunctionNotFoundException e) {
            long duration = System.currentTimeMillis() - startTime;

            log.error("Tool [{}] {} not found after {}ms", index, toolName, duration, e);

            return ToolResult.error(call.getId(), toolName, "Function not found: " + toolName, duration);

        } catch (FunctionExecutionException e) {
            long duration = System.currentTimeMillis() - startTime;

            log.error("Tool [{}] {} failed after {}ms: {}", index, toolName, duration, e.getMessage(), e);

            return ToolResult.error(call.getId(), toolName, e.getMessage(), duration);
        }
    }
     ...
     *
     * <p>Currently executes sequentially. Future enhancement could analyze dependencies and
     ...
     *
     * <p>All tools execute simultaneously. Use with caution - only for tools with no dependencies on
     * each other.
     *
     * @param toolCalls List of tool calls to execute
     * @return List of tool results
    ...
    public List<ToolResult> executeParallel(List<ToolCall> toolCalls) {
        ...
        for (int i = 0; i < toolCalls.size(); i++) {
            ...
            CompletableFuture<ToolResult> future =
                    CompletableFuture.supplyAsync(() -> executeSingle(call, index), executorService);
    ...
    /**
     * Shutdown the executor service. Call this when done with the executor.
     */
