 ...
 * Provides strategies for trimming messages when context is too large.
 */
@Slf4j
public class ContextWindowManager {
    ...
    /**
     * Fit messages to a maximum token count by removing oldest messages.
     ...
     * 3. Remove oldest messages until under target
     * 
     * @param messages Original messages
    ...
    public List<Message> trimOldest(List<Message> messages, int targetTokens) {
        ...
        log.info("Trimmed from {} to {} messages ({} tokens)", 
                 messages.size(), result.size(), currentTokens);
