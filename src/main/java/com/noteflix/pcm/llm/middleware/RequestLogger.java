 ...
 *
 * <p>Logs requests, responses, and metrics for monitoring and debugging.
 *
 * <p>Features: - Request/response logging - Performance metrics - Token usage tracking - Cost
...
public class RequestLogger {
  ...
  // Metrics storage
  private final ConcurrentHashMap<String, RequestMetrics> activeRequests =
  ...
  @Builder.Default private boolean logRequests = true;

  /** Enable response logging Default: true */
  @Builder.Default private boolean logResponses = true;

  /** Log full request/response content (can be large) Default: false (only log summaries) */
  @Builder.Default private boolean logFullContent = false;

  /** Track metrics (tokens, latency, costs) Default: true */
  @Builder.Default private boolean trackMetrics = true;

  /** Enable error logging Default: true */
  @Builder.Default private boolean logErrors = true;
  ...
  public String logRequest(String provider, LLMRequest request) {
    ...
    if (logRequests) {
      if (logFullContent) {
        log.info(
            "🚀 [{}] Request to {}: model={}, messages={}, temp={}",
            requestId,
            provider,
            request.getModel(),
            request.getMessages(),
  ...
  public void logResponse(String requestId, LLMResponse response) {
    ...
    if (logResponses) {
      if (logFullContent) {
        log.info(
            "✅ [{}] Response: content={}, tokens={}, finishReason={}",
            requestId,
            response.getContent(),
    ...
    if (trackMetrics && metrics != null) {
      ...
      log.info(
          "📊 [{}] Metrics: latency={}ms, tokens={}, provider={}, model={}",
          requestId,
          latencyMs,
