...
public class RetryPolicy {
  ...
  @Builder.Default private Duration initialDelay = Duration.ofSeconds(1);

  /** Maximum delay between retries Default: 30 seconds */
  @Builder.Default private Duration maxDelay = Duration.ofSeconds(30);
  ...
  @Builder.Default private boolean useJitter = true;

  /**
   * Predicate to determine if an exception should be retried Default: retry on LLMProviderException
   * with 5xx status codes
   */
  @Builder.Default private Predicate<Throwable> retryCondition = RetryPolicy::isRetryableException;
