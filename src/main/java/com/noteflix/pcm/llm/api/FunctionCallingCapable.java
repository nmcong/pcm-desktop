 ...
 *
 * <p>Function calling allows the LLM to: 1. Understand available functions/tools 2. Decide when to
 * call them based on user input 3. Generate structured arguments for the function
 *
 * <p>Follows SOLID principles: - Interface Segregation: Separated capability - Single
 * Responsibility: Only handles function calling
 *
 * <p>Design Pattern: Strategy Pattern Different providers implement function calling differently
 *
 * <p>Workflow: 1. User asks: "What's the weather in Tokyo?" 2. LLM decides to call
 * get_weather("Tokyo") 3. Your code executes get_weather function 4. Send result back to LLM 5. LLM
...
public interface FunctionCallingCapable {
   ...
   *
   * <p>The LLM can decide to: - Answer directly (normal response) - Call one or more functions
  ...
  /**
   * Check if function calling is supported May depend on model or configuration
   *
  ...
  /**
   * Get maximum number of functions supported in single request Some providers limit the number of
   * functions
   *
