    ...
    private static void demonstrateCodeAnalysis() {
        ...
        if (result.isSuccessful()) {
            ...
            for (ASTAnalysisResult.ClassInfo classInfo : result.getClasses()) {
                log.info("Class: {} (public: {}, interface: {})", 
                    classInfo.getName(), classInfo.isPublic(), classInfo.isInterface());
            }
            
            for (ASTAnalysisResult.MethodInfo methodInfo : result.getMethods()) {
                log.info("Method: {} returns {} (public: {}, static: {}, params: {})", 
                    methodInfo.getName(), methodInfo.getReturnType(), 
                    methodInfo.isPublic(), methodInfo.isStatic(), methodInfo.getParameterCount());
            }
        } else {
